  ...
  <body>
    ...
    <div class="form-group" style="clear: both;height:auto;overflow:hidden">
  	...
  	<div style="text-align:center;margin:25px 0">
  	    <button class="btn btn-success" id="success_examine">审核通过</button>
  	    <button class="btn btn-success" style="margin-left:50px" id="fail_examine">审核不通过</button>
  	    <button class="btn btn-default" style="margin-left:50px" id="cancel_examine">取消</button>
  	</div>
    ...
    //审核通过
    $('#success_examine').on('click',function(){console.log(Information);
    	$.ajax({
    				...
    				if(res.errMsg == '大置顶冲突!'){
    					swal({
    						...
    						layer.open({
        			            ...
        			            title: '查看或修改冲突大置顶时间',
        			            shadeClose: true,
        			            content: '<%=basePath%>admin/feedtopconflict.shtml?obj_id='+page.id+'&obj_type=7&start_time='+getValueTime(Information.bigTop.startTime)+'&end_time='+getValueTime(Information.bigTop.endTime),
        			            area: [ '900px', '700px' ],
    ...
    //审核不通过
    $('#fail_examine').on('click',function(){console.log(Information);
    	$.ajax({
    		url: '<%=basePath%>admin/article/audit_article.shtml',
    		data: {
    			id: page.id,
    ...
    //取消
    $('#cancel_examine').on('click',function () {
    	parent.layer.close(parent.layer.index);
    });
    </script>
  </body>
