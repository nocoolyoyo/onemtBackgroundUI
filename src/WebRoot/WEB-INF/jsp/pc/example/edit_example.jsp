...
<html lang="zh_cn">
  ...
  <body>
    ...
    $('#abc').on('click',function(){
    	console.log('信息流',Information);
    	console.log('设备',equipment);
    });
    //信息流推送
    var Information = {
    ...
    $.ajax({
    	...
    	success: function(res){console.log(res);
    		...
    		for(var i = 0; i < res.pushdevice.length ;i++){
				if(res.pushdevice[i] && res.pushdevice[i].is_push_type == 1) setInformation(res.pushdevice[i]);
				if(res.pushdevice[i] && res.pushdevice[i].is_push_type == 2) setEquipment(res.pushdevice[i]);
			}
    	},
    	error: function(xhr){
    		console.log(xhr);
