...
<html lang="zh_cn">
  ...
  <body>
    ...
    <div class="form-group" style="clear: both;height:auto;overflow:hidden">
      ...
      <div style="text-align:center;margin:25px 0">
          <button type="button" class="btn btn-primary" id="success_examine" style="margin-right:50px">审核通过</button>
          <button type="button" class="btn btn-primary" id="fail_examine" style="margin-right:50px">审核不通过</button>
          <button type="button" class="btn btn-white" data-dismiss="modal" id="close_examine">取消</button>
      </div>
  </body>
  ...
  <script>
    				...
    				if(res.errMsg == '大置顶冲突!'){
    					swal({
    						...
    						layer.open({
        			            ...
        			            title: '查看或修改冲突大置顶时间',
        			            shadeClose: true,
        			            content: '<%=basePath%>admin/feedtopconflict.shtml?obj_id='+page.id+'&obj_type='+2+'&start_time='+getValueTime(Information.bigTop.startTime)+'&end_time='+getValueTime(Information.bigTop.endTime),
        			            area: [ '900px', '700px' ],
    ...
    //审核不通过
    $('#fail_examine').on('click',function(){
    	$.ajax({
    		url: '<%=basePath%>admin/secretNews/check_pendingauditsecretnews.shtml',
    		data: {
    			id: page.id,
