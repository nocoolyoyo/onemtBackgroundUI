...
(function (factory) {
    ...
    function refreshDropMenu($input, data, options) {
        ...
        var fields = options.effectiveFields;   //Created by xiegy on 2017/5/18. 按指定effectiveFields顺序排序


        /**
         * Created by xiegy on 2017/5/18. 添加显示数量的限制
         */
        len = data.value.length < options.showMaxCount ? data.value.length : options.showMaxCount;
        if (!data || !len) {
        ...
        if (options.showHeader) {
            ...
            /**
             * Created by xiegy on 2017/5/18. 按指定effectiveFields顺序排序
             */
            if(!fields || fields.length == 0){
                ...
                for(i = 0; i < fields.length; i++){
                    html.push('<th>', (options.effectiveFieldsAlias[fields[i]] || fields[i]),
                        i === 0 ? ('(' + len + ')') : '' , // 表头第一列记录总数
                        '</th>');
                }
            }
        ...
        for (i = 0; i < len; i++) {
            ...
            if(!fields || fields.length == 0){
                ...
                for(index = 0; index < fields.length; index++){
                    if(dataI[fields[index]] == undefined) dataI[fields[index]] = "";
                    tds.push('<td data-name="', fields[index], '">', dataI[fields[index]], '</td>');
                }

                // 标记作为 value 和 作为 id 的值
                if (!keyValue && options.indexKey >= 0 && options.indexKey < fields.length) {
                    keyValue = dataI[fields[options.indexKey]].replace(/"/g, ";quot");
                }
                if (!idValue && options.indexId >= 0 && options.indexId < fields.length) {
                    idValue = dataI[fields[options.indexId]].replace(/"/g, ";quot");
                }
            }
    ...
    var defaultOptions = {
        ...
        showMaxCount: 50,               // Created by xiegy on 2017/5/18. 新增最多显示数量的限制
