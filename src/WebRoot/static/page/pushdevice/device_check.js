...
//页面所用到AJAX请求的URL
page.CONFIG = {
    GET_LIST: helper.url.getUrlByMapping("admin/pushDevice/find_pushdevicelist.shtml?state=2&status=1"),     //查询待审核设备推送接口
    DELETE: helper.url.getUrlByMapping("admin/pushDevice/update_pushdevicestatus.shtml"),           //删除设备推送接口
    PUSH_USER: helper.url.getUrlByMapping("admin/pushmemberindex.shtml"),                      //查看推送用户
    UPDATE: helper.url.getUrlByMapping("admin/addDevice.shtml"),                      //查看编辑设备推送
...
//                    SAVETYPE: 2
    ...
    showEdit: function (id) {
        helper.win.open({name: "编辑用户", url: page.CONFIG.UPDATE + "?action= edit" + "&id=" + id});
    },
    
    //打开编辑
    showPushUser: function (id) {
        helper.win.open({name: "查看推送用户", url: page.CONFIG.PUSH_USER + "?id=" + id});
    },
    ...
    page.$table.bootstrapTable({
        ...
        columns: [{
            ...
            formatter: function(value, row){
            	var title = ''; 
            	if(value == undefined){
            		title = '<a href="#" class="paperDetail" onclick = "page.eventHandler.showEdit('+row.id+')">_</a>';
            	}else{
            		title = '<a href="#" class="paperDetail" onclick = "page.eventHandler.showEdit('+row.id+')">' + value + '</a>';
            	}
        ...
        },
        {
            ...
            width: "250px",
            formatter:function(value, row){
                var strHtml=' <button type="button" class="btn btn-sm btn-info" onclick="page.eventHandler.showAudit(' + row.id + ')">审核</button>';
