...
page.CONFIG = {
    ...
    GET_UNIT_API: helper.url.getUrlByMapping("admin/business/get_business_unit_id.shtml"),     		 //获取招商单位列表的接口
    GET_INFO_API: helper.url.getUrlByMapping("admin/business/get_business_id.shtml"),                //获取招商项目接口
    ADD_API: helper.url.getUrlByMapping("admin/business/insert_business.shtml"),                     //新增招商项目接口
    EDIT_API: helper.url.getUrlByMapping("admin/business/update_business_id.shtml"),      			 //修改招商项目接口
    ...
    STATE_LIST: {"edit": {action: "edit", title: "编辑招商项目"},
        "add": {action: "add", title: "新增招商项目"},
        "audit": {action: "audit", title: "审核修改招商项目"},
        "release": {action: "release", title: "已发布招商项目修改"}
    },     //页面所有状态
    RELATION: "businessunitproject.shtml",                                        //关联的打开窗口
...
page.eventHandler = {
    ...
    initUnit: function () {
        //初始化招商单位
        $.ajax({
            url: page.CONFIG.GET_UNIT_API+"?id="+page.unitid,
            async:false,
            type: 'post',
            dataType: 'json',
            success: function(res){
                if(res.code == 0){
                    page.unitname = res.data.title;
                    page.unit_avatar = res.data.avatar;
                    page.unit_profile = res.data.profile;
            ...
            error: function(){
                toastr.error("OMG!", "获取招商单位信息错误，请稍候再重试！");
            }
        });
    },
    ...
    initPageInfo: function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                toastr.error("待编辑的招商项目不存在或发生错误!", "请稍候再重试或联系管理员！");
            },
            error: function () {
                toastr.error("待编辑的招商项目不存在或发生错误!", "请稍候再重试或联系管理员！");
            }
        ...
        if(bv.isValid()){
            ...
            $.ajax({
                ...
                success : function(ret) {
                    if(ret.code == 0){
                    	helper.win.changeQuoto({msg: stateInfo.msg, relation: page.ref+"?status=1&state=1&unitid="+page.unitid});
                    }else{
...
page.assist = {
    ...
    getParams: function (state) {
        var data = {};
        data.unit_id=page.unitid,
        data.unit_name=page.unitname,
        data.unit_avatar=page.unit_avatar,
        data.unit_profile=page.unit_profile,
