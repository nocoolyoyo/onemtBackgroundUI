...
//页面所用到配置
page.CONFIG = {
    GET_LIST_API: helper.url.getUrlByMapping("admin/business/find_business_unit.shtml?status=1"),    		//查询已发布招商单位列表接口
    ...
    EDIT_URL: helper.url.getUrlByMapping("admin/businessunitadd.shtml"),                                	//编辑页面
...
page.eventHandler = {
	//打开新增分享
	showAdd: function () {
		helper.win.open({name: "新增招商单位", url: page.CONFIG.ADD_URL});
	},
    //查看项目
    showProject: function (id) {
        var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
        var title = rowDate.title.substr(0, 5) + "..的项目"
        helper.win.open({name: title,url: page.CONFIG.PROJECT_URL+"&unitid="+id});
    },

    //查看关注的用户
    showUser: function (id) {
        var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
        var title = rowDate.title.substr(0, 5) + "..的关注用户"
        helper.win.open({name: title,url: page.CONFIG.USER_URL+"?unitid="+id});
    },
    //查看单位详情
    show: function (id) {
        helper.win.open({name: "查看单位信息",url: page.CONFIG.SHOW_URL+"?action=show&id="+id});
    },

    //打开编辑
    showEdit: function (id) {
        helper.win.open({name: "编辑单位信息",url: page.CONFIG.EDIT_URL+"?action=edit&id="+id});
    },
