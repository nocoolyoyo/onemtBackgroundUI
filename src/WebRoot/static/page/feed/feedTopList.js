...
require(['base', 'jquery', 'helper', 'sweetalert', 'layer', 'toastrPlus', 'module.push', 'datetimepicker', 'table', 'tableEditable'], function (bs, $, helper, swal, layer, toastr) {
  ...
  //页面所用到配置page.CONFIG.SUBMIT_AUDIT_API
    page.CONFIG = {
    	...
    	GET_LIST_API: helper.url.getUrlByMapping("admin/feed/find_feed_tmp.shtml"),                    //查询大小置顶列表接口
    ...
    $(document).ready(function () {
        ...
        page.$table.bootstrapTable({
            ...
            columns: [{
            	...
            	title: '操作',
            	align: 'center',
            	width: page.type == 'small' ? '320px' : '200px',
            	formatter:function(value, row){
                    var strHtml= '';
                    switch (page.type){
                        case page.CONFIG.TYPE_LIST.small.type:
                            //小置顶列表
                            strHtml += '&nbsp;<button type="button" class="btn btn-sm btn-info" onclick="page.eventHandler.showPushUser(' + row.feed_id + ')">查看推送用户</button>';
                            strHtml += '&nbsp;<button type="button" class="btn btn-sm btn-warning" onclick="page.eventHandler.stopPush('+row.id+')">停止推送</button>';
                            strHtml += '&nbsp;<button type="button" class="btn btn-sm btn-warning" onclick="page.eventHandler.editTime('+row.id+','+row.start_time+','+row.end_time+')">修改时间</button>';
                            strHtml += '&nbsp;<button type="button" class="btn btn-sm btn-danger" onclick="page.eventHandler.delete(' + row.id + ',' + row.feed_id + ')">删除</button>';
                            break;
                        case page.CONFIG.TYPE_LIST.big.type:
                            //大置顶列表
