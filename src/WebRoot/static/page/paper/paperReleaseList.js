...
require(['base', 'jquery', 'helper', 'sweetalert', 'layer', 'toastrPlus', 'module.push', 'datetimepicker', 'table', 'tableEditable', 'validator'], function (bs, $, helper, swal, layer, toastr) {
  ...
  //页面所用到配置
    page.CONFIG = {
        GET_LIST_API: helper.url.getUrlByMapping("admin/morning/find_morning.shtml?state=1&status=1"),    //查询已发布早茶列表接口
        SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/morning/update_morning_id.shtml"),    //取消发布接口（早茶审核接口）
        ...
        HANDLE_URL: helper.url.getUrlByMapping("admin/paperhandle.shtml?action=release"),                  //修改早茶url
        SHOW_URL: helper.url.getUrlByMapping("admin/paperinfo.shtml?action=release"),                      //查看早茶url
        OBJ_TYPE: 1,            //早茶的类型id
    ...
    page.eventHandler = {
        ...
        showPush: function (id) {
            ...
            layer.open({
                type: 1,
                title: '<span class="label label-warning">【早茶】{0}</span>&nbsp;&nbsp;的再次推送'.Format(rowDate.title),
        ...
        doHandler: function (id, data, api, label) {
            var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
            swal({
                title: "您确定要{0}选中的早茶吗？".Format(label),
                text: rowDate.title,
                type: "warning",
        ...
        page.$table.bootstrapTable({
            ...
            columns: [{
                ...
                formatter:function(value){ 
                	return helper.convert.formatDate(value, 'yyyy-MM-dd');
                },
            },{
