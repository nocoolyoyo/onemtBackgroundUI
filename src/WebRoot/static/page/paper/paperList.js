...
require(['base', 'jquery', 'helper', 'sweetalert', 'layer', 'datetimepicker', 'table'], function (bs, $, helper, swal, layer) {
	//页面所用到配置
	page.CONFIG = {
	    GET_LIST_API: helper.url.getUrlByMapping("admin/morning/find_morning.shtml"),                     //查询早茶列表接口
	    SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/morning/update_morning_id.shtml"),            //提交审核接口
	    DELETE_API: helper.url.getUrlByMapping("admin/morning/delete_morning_id.shtml"),                  //删除早茶接口
	    HANDLE_URL: helper.url.getUrlByMapping("admin/paperhandle.shtml"),                                //新增/审核/修改早茶url
	    SHOW_URL: helper.url.getUrlByMapping("admin/paperinfo.shtml"),                                    //查看/审核早茶url
	    OBJ_TYPE: 1,            //早茶的类型id
	    ...
	    TYPE_LIST: {"wait": {type: "wait", param: "status=1&state=2", title: "待审核早茶", action: "audit"},
	        "draft": {type: "draft", param: "status=1&state=3,4", title: "早茶草稿箱", action: ""},
	        "trash": {type: "trash", param: "status=2", title: "早茶垃圾箱", action: ""}
	    }      //页面全部状态
	};
	...
	page.derive = {
	    ...
	    getParams: function (params) {
	        params.start_periods = helper.convert.formatTimestamp($("#startTime").val());
	        params.end_periods = helper.convert.formatTimestamp($("#endTime").val(), {day: 1});
	...
	page.eventHandler = {
	    ...
	    reply: function (id) {
	        ...
	        page.eventHandler.doHandler(id, data, page.CONFIG.SUBMIT_AUDIT_API, "恢复");
	    },
