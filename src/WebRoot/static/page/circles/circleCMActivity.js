...
//页面所用到AJAX请求的URL
page.CONST = {
    GET_LIST: helper.url.getUrlByMapping("admin/circle/circle_activity.shtml"),     						    //获取全部圈子活动接口
    UPDATE: helper.url.getUrlByMapping("admin/circle/update_circle_related.shtml"),                 			//设置圈子活动状态接口(置顶，精华，删除)   
    HANDLE_URL: helper.url.getUrlByMapping("admin/circleActivityHandle.shtml"),                                 //新增/修改活动url
...
page.eventHandler = {
	//打开新增活动
	showAdd: function () {console.log(page.CONST.HANDLE_URL);
	    helper.win.open({name: "新增活动", url: page.CONST.HANDLE_URL});
	},
	//评论管理
	showComment: function (id){
	    var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
	    var title = rowDate.title.substr(0, 5) + "..评论";
	    helper.win.open({name: title, url: page.CONST.COMMENT_URL + "?obj_id=" + id + "&obj_type=4"});
	},
    //打开直播
    showLive: function (id) {
        var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
        var title = rowDate.title.substr(0, 5) + "..的直播";
        helper.win.open({name: title, url: page.CONST.Live_URL + "?page_id=" + id + "&page_title=" + rowDate.title});
    },
	//搜索
    search: function () {
