...
//页面所用到AJAX请求的URL
page.CONST = {
    GET_LIST: helper.url.getUrlByMapping("admin/circle/circle_topic.shtml"),     						    //获取全部圈子文章接口
    HANDLE_URL: helper.url.getUrlByMapping("admin/circleTopicHandle.shtml"),                             	//新增/修改活动url
...
page.eventHandler = {
	//打开新增话题
	showAdd: function () {
		console.log(page.CONST.HANDLE_URL);
		helper.win.open({name: "新增话题", url: page.CONST.HANDLE_URL});
	},
	//评论管理
	showComment: function (id){
		var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
		var title = rowDate.title.substr(0, 5) + "..评论";
		helper.win.open({name: title, url: page.CONST.COMMENT_URL + "?obj_id=" + id + "&obj_type=5"});
	},
	//搜索
    search: function () {
