...
//页面所用到AJAX请求的URL
page.ajaxUrl = {
    GET_LIST: helper.url.getUrlByMapping("admin/circle/circle_share.shtml"),               //获取全部圈子文章接口
    HANDLE_URL: helper.url.getUrlByMapping("admin/circlesharehandle.shtml"),                                 //新增/修改帖子url
...
page.eventHandler = {
	//打开新增帖子
	showAdd: function () {
		helper.win.open({name: "新增帖子", url: page.ajaxUrl.HANDLE_URL});
	},
	//评论管理
	showComment: function (id){
		var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
		var title = rowDate.title.substr(0, 5) + "..评论";
		helper.win.open({name: title, url: page.ajaxUrl.COMMENT_URL + "?obj_id=" + id + "&obj_type=19"});
	},
	//搜索
    search: function () {
