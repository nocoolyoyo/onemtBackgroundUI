...
page.eventHandler = {
	showAdd: function () {
		helper.win.open({name: "新增用户组",url: "weightsadd.shtml"});
	},
	showUpdata: function (id) {
		helper.win.open({name: "编辑用户权重",url: "weightsedit.shtml?id="+id});
	},
	delete: function (id) {
	    var rowDate = $('#tableList').bootstrapTable('getRowByUniqueId', id);
	    swal({
	        title: "您确定要删除选中的信息吗？",
	        text: rowDate.weightsname,
	        type: "warning",
    		...
    		$.ajax({
	    		url: 'weights/delete_weights.shtml',
	    		data: {
	    			id: id
	    		},
	    		type: 'post',
...
$table.bootstrapTable({
    //请求相关
    url: helper.url.getUrlByMapping('admin/weights/find_weightslists.shtml'),  //AJAX读取列表数据的URL
    method: "get",                  //请求方式
