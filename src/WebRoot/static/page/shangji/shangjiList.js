...
//页面所用到配置
page.CONFIG = {
    GET_LIST_API: helper.url.getUrlByMapping("admin/business/find_business_id.shtml"),                  //查询商机列表接口
    SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/business/update_business_id.shtml"),    		//提交审核接口
    DELETE_API: helper.url.getUrlByMapping("admin/business/delete_business_id.shtml"),                        //删除商机接口
    HANDLE_URL: helper.url.getUrlByMapping("admin/shangjiadd.shtml"),                                 	//新增/审核/修改商机url
    ...
    TYPE_LIST: {"wait": {type: "wait", param: "status=1&state=2", title: "待审核商机", action: "audit"},
        "draft": {type: "draft", param: "status=1&state=3,4", title: "商机草稿箱", action: ""},
        "trash": {type: "trash", param: "status=2", title: "商机垃圾箱", action: ""}
    }      //页面全部状态
};
...
page.eventHandler = {
    //打开新增商机
    showAdd: function () {
        helper.win.open({name: "新增商机", url: page.CONFIG.HANDLE_URL});
    },

    //打开审核
    showAudit: function (id) {
        helper.win.open({name: "审核商机", url: page.CONFIG.SHOW_URL + "?action=" + page.typeInfo.action + "&id=" + id});
    },

    //打开编辑
    showEdit: function (id) {
        var action = page.typeInfo.action ? page.typeInfo.action : "edit";
        helper.win.open({name: "编辑商机", url: page.CONFIG.HANDLE_URL + "?action=" + action + "&id=" + id});
    },
...
$(document).ready(function () {
    ...
    page.$table.bootstrapTable({
        ...
        columns: [{
            ...
            field: 'update_time',
            title: page.type == page.CONFIG.TYPE_LIST.trash.type ? '删除时间' : '最后修改时间',
            width: "150px",
            formatter: function(value){
                return helper.convert.formatDate(value);
            },
            ...
            formatter: function (value, row) {
                ...
                if(value == page.CONFIG.NOPASS_STATE)
                    state = '<span class="text-danger" onmouseenter="page.eventHandler.showTip(this)" data-tip="审核意见：{1}">{0}</span>'.Format(state, row.audit_opinion?row.audit_opinion:"无");
