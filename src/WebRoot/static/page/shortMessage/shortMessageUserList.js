...
require(['base', 'jquery', 'helper', 'table'], function (bs, $, helper) {
  //页面所用到配置
    page.CONFIG = {
        GET_LIST_API: helper.url.getUrlByMapping("admin/sendMessage/find_sendmessagememberlists.shtml")          //查询发送短信用户列表接口
    };
    ...
    page.derive = {
        ...
        getParams: function (params) {
            params.message_id = page.id;
            params.type = helper.url.queryString("type");
            params.realname = $("#keyword").val();
    ...
    $.ajax({
    	url: page.CONFIG.GET_LIST_API + '?message_id=' + page.id,
    	type: 'post',
    	dataType: 'json',
    	success: function (ret) {
    		console.log(ret);
