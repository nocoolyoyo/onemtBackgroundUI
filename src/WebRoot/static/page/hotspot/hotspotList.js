...
require(['base', 'jquery', 'helper', 'sweetalert', 'layer', 'datetimepicker', 'table'], function (bs, $, helper, swal, layer) {
	//页面所用到配置
	page.CONFIG = {
	    GET_LIST_API: helper.url.getUrlByMapping("admin/hotspots/find_hotspots.shtml"),                     //查询专题列表接口
	    SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/hotspots/audit_hotspots_id.shtml"),    //提交审核接口
	    DELETE_API: helper.url.getUrlByMapping("admin/hotspots/delete_hotspots_id.shtml"),                        //删除专题接口
	    HANDLE_URL: helper.url.getUrlByMapping("admin/hotspothandle.shtml"),                                 //新增/审核/修改专题url
	    SHOW_URL: helper.url.getUrlByMapping("admin/hotspotinfo.shtml"),                                     //查看/审核专题url
	    OBJ_TYPE: 6,            //专题的类型id
	    ...
	    TYPE_LIST: {"wait": {type: "wait", param: "status=1&state=2", title: "待审核专题", action: "audit"},
	        "draft": {type: "draft", param: "status=1&state=3,4", title: "专题草稿箱", action: ""},
	        "trash": {type: "trash", param: "status=2", title: "专题垃圾箱", action: ""}
	    }      //页面全部状态
	};
