...
require(['base', 'jquery', 'helper', 'toastr', 'sweetalert', 'module.push', 'module.articleSelector', 'fancybox'], function (bs, $, helper, toastr, swal) {
  ...
  //页面所用到配置
    page.CONFIG = {
        GET_ANONYMITY_API: helper.url.getUrlByMapping(""),     //获取专题链接接口
        ...
        UPDATE_URL: helper.url.getUrlByMapping("admin/hotspothandle.shtml"),                   //在审核状态进入修改专题url
    ...
    page.eventHandler = {
        ...
        initPageInfo: function () {
            $.ajax({
                ...
                success: function (res) {
                    ...
                    toastr.error("您查看的专题不存在或发生错误!", "请稍候再重试或联系管理员！");
                },
                error: function () {
                    toastr.error("您查看的专题不存在或发生错误!", "请稍候再重试或联系管理员！");
                }
        ...
        delete: function (status) {
            ...
            swal({
                title: "您确定要{0}该专题吗？".Format(label),
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#DD6B55",
    ...
    page.assist = {
        ...
        //初始化专题链接
        setHotspotLink: function (linkData) {
        	var str = '';
        	if (linkData){
    ...
    //相关报道表格初始化
    /*page.$relationArtTable.bootstrapTable({
        detailView: false,
        // classes: 'table table-hover table-no-bordered',
        data: page.hostpotLink,
