...
require(['base', 'jquery', 'helper', 'sweetalert', 'layer', 'toastrPlus', 'module.push', 'datetimepicker', 'table', 'tableEditable', 'validator'], function (bs, $, helper, swal, layer, toastr) {
    ...
    //页面所用到配置
    page.CONFIG = {
        GET_LIST_API: helper.url.getUrlByMapping("admin/hotspots/find_hotspots.shtml?state=1&status=1"),    //查询已发布专题列表接口
        SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/hotspots/audit_hotspots_id.shtml"),    //取消发布接口（专题审核接口）
        DELETE_API: helper.url.getUrlByMapping("admin/hotspots/delete_hotspots_id.shtml"),                        //删除专题接口
        ...
        HANDLE_URL: helper.url.getUrlByMapping("admin/hotspothandle.shtml?action=release"),                  //修改专题url
        SHOW_URL: helper.url.getUrlByMapping("admin/hotspotinfo.shtml?action=release"),                      //查看url
        OBJ_TYPE: 6,            //专题的类型id
    ...
    page.derive = {
        ...
        getParams: function (params) {
            params.astart_time = helper.convert.formatTimestamp($("#releaseStartTime").val());
            params.aend_time = helper.convert.formatTimestamp($("#releaseEndTime").val(), {day: 1});
    ...
    page.eventHandler = {
        ...
        showPush: function (id) {
            ...
            layer.open({
                type: 1,
                title: '<span class="label label-warning">【专题】{0}</span>&nbsp;&nbsp;的再次推送'.Format(rowDate.title),
        ...
        doHandler: function (id, data, api, label) {
            var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
            swal({
                title: "您确定要{0}选中的专题吗？".Format(label),
                text: rowDate.title,
                type: "warning",
