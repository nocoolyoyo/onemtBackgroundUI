...
require(['base', 'jquery', 'helper', 'sweetalert', 'layer', 'datetimepicker', 'table'], function (bs, $, helper, swal, layer) {
	//页面所用到配置
	page.CONFIG = {
	    GET_LIST_API: helper.url.getUrlByMapping("admin/example/find_example.shtml"),                     //查询榜样列表接口
	    SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/example/audit_example.shtml"),                //提交审核接口
	    DELETE_API: helper.url.getUrlByMapping("admin/example/update_example.shtml"),                        //删除榜样接口
	    HANDLE_URL: helper.url.getUrlByMapping("admin/exampleHandle.shtml"),                                 //新增/审核/修改榜样url
	    SHOW_URL: helper.url.getUrlByMapping("admin/exampleinfo.shtml"),                                     //查看/审核url
	    OBJ_TYPE: 8,             //榜样的对象类型
	    ...
	    TYPE_LIST: {"wait": {type: "wait", param: "status=1&state=2", title: "待审核榜样", action: "audit"},
	        "draft": {type: "draft", param: "status=1&state=3,4", title: "榜样草稿箱", action: ""},
	        "trash": {type: "trash", param: "status=2", title: "榜样垃圾箱", action: ""}
	    }      //页面全部状态
	};
