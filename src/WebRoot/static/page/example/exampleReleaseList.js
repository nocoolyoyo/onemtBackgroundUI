...
require(['base', 'jquery', 'helper', 'sweetalert', 'layer', 'toastrPlus', 'module.push', 'datetimepicker', 'table', 'tableEditable', 'validator'], function (bs, $, helper, swal, layer, toastr) {
	...
	//页面所用到配置
	page.CONFIG = {
	    GET_LIST_API: helper.url.getUrlByMapping("admin/example/find_example.shtml?state=1&status=1"),    //查询已发布榜样列表接口
	    SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/example/audit_example.shtml"),                //取消发布接口（榜样审核接口）
	    ...
	    HANDLE_URL: helper.url.getUrlByMapping("admin/exampleHandle.shtml?action=release"),                  //修改榜样url
	    SHOW_URL: helper.url.getUrlByMapping("admin/exampleinfo.shtml?action=release"),                      //查看榜样url
	    OBJ_TYPE: 8,            //榜样的类型id
	...
	page.eventHandler = {
	    ...
	    showPush: function (id) {
	        ...
	        layer.open({
	            type: 1,
	            title: '<span class="label label-warning">【榜样】{0}</span>&nbsp;&nbsp;的再次推送'.Format(rowDate.title),
