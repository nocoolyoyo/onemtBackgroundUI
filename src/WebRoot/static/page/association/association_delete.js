...
//页面所用到AJAX请求的URL
page.CONFIG = {
    GET_LIST: helper.url.getUrlByMapping("admin/association/find_article_association.shtml?status=2"),      //查询待审核内容接口
...
page.derive = {
    ...
    getParams: function (params) {
        ...
        params.ustart_time = helper.convert.formatTimestamp($("#startTime1").val());;
        params.uend_time = helper.convert.formatTimestamp($("#endTime1").val(), {day: 1});
...
$(document).ready(function () {
    ...
    $('.form_date').datetimepicker({
        ...
        if($this.attr("start_time")){
            $this.datetimepicker("setStartDate", $($this.attr("start_time")).val());
        }
        if($this.attr("endTime")){
            $this.datetimepicker("setEndDate", $($this.attr("endTime")).val());
        }
        if($this.attr("startTime1")){
            $this.datetimepicker("setStartDate", $($this.attr("startTime1")).val());
        }
        if($this.attr("endTime1")){
            $this.datetimepicker("setEndDate", $($this.attr("endTime1")).val());
        }
    });
    ...
    page.$table.bootstrapTable({
        ...
        columns: [{
            ...
            formatter:function(value, row){
                var strHtml=' <button type="button" class="btn btn-sm btn-danger" onclick="page.eventHandler.delete('+row.id+',' + row.obj_id + ','+row.obj_type+')">恢复</button>';
                return strHtml;
            }
        }]
    });
