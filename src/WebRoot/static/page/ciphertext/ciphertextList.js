...
require(['base', 'jquery', 'helper', 'sweetalert', 'layer', 'datetimepicker', 'table'], function (bs, $, helper, swal, layer) {
    //页面所用到配置
    page.CONFIG = {
        GET_LIST_API: helper.url.getUrlByMapping("admin/secretNews/find_secretlist.shtml"),                     //查询秘闻列表接口
        SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/secretNews/check_pendingauditsecretnews.shtml"),    //提交审核接口
        ...
        TYPE_LIST: {"wait": {type: "wait", param: "status=1&state=2", title: "待审核秘闻", action: "audit"},
            "draft": {type: "draft", param: "status=1&state=3", title: "秘闻草稿箱", action: ""},
            "trash": {type: "trash", param: "status=2", title: "秘闻垃圾箱", action: ""}
        }      //页面全部状态
    };
