...
require(['base', 'jquery', 'helper', 'sweetalert', 'layer', 'toastrPlus', 'module.push', 'datetimepicker', 'table', 'tableEditable', 'validator'], function (bs, $, helper, swal, layer, toastr) {
    ...
    //页面所用到配置
    page.CONFIG = {
        GET_LIST_API: helper.url.getUrlByMapping("admin/secretNews/find_secretlist.shtml?state=1&status=1"),    //查询已发布秘闻列表接口
        SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/secretNews/check_pendingauditsecretnews.shtml"),    //取消发布接口（秘闻审核接口）
    ...
    page.eventHandler = {
        ...
        showPush: function (id) {
            ...
            layer.open({
                type: 1,
                title: '<span class="label label-warning">【秘闻】{0}</span>&nbsp;&nbsp;的再次推送'.Format(rowDate.title),
        ...
        doHandler: function (id, data, api, label) {
            var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
            swal({
                title: "您确定要{0}选中的秘闻吗？".Format(label),
                text: rowDate.title,
                type: "warning",
    ...
    $(document).ready(function () {
        ...
        page.$table.bootstrapTable({
            ...
            columns: [{
                ...
                field: 'true_count_all',
                title: '投票真',
                width: '40px',
                align: 'center',
                editable: page.derive.getEditable('修改投票真的数量', 'true_count_all')
            },{
                field: 'false_count_all',
                title: '投票假',
                width: '40px',
                align: 'center',
                editable: page.derive.getEditable('修改投票假的数量', 'false_count_all')
            },{
                ...
                width: '40px',
                align: 'center'
//                editable: page.derive.getEditable('修改微信访问人次的数量', 'wx_visit_times_count')
            },{
                field: 'wx_visit_people_count',
                title: '点击人<br/><small>(微信)</small>',
                width: '40px',
                align: 'center'
//                editable: page.derive.getEditable('修改微信访问人数的数量', 'wx_visit_people_count')
            },{
                field: 'comment_count',
                ...
                field: 'true_count',
                title: '投票真<br/><small>(真实)</small>',
                width: '40px',
                visible: false,
                align: 'center'
            },{
                field: 'false_count',
                title: '投票假<br/><small>(真实)</small>',
                width: '40px',
                visible: false,
                align: 'center'
            },{
