...
require(['base', 'jquery', 'helper', 'iCheckPlus', 'toastr', 'helper.qiniu', 'module.editor', 'module.push', 'module.fileUpload', 'module.articleSelector', 'validator'], function (bs, $, helper, iCheck, toastr) {
	...
	//页面所用到AJAX请求的URL
page.CONFIG = {
	GET_INFO_API: helper.url.getUrlByMapping("admin/shanghui/find_information_detail.shtml"),                //获取江湖事接口
...
page.eventHandler = {
    ...
    initPageInfo: function () {
        ...
        $.ajax({
            ...
            data: {INID: page.id},
            dataType: 'json',
            success: function (res) {console.log('zixun',res.data);
                if(res.code == 0){
...
//                url = page.CONFIG.EDIT_URL;
            ...
            $.ajax({
                ...
                success : function(ret) {
                    if(ret.code == 0){
                    	helper.win.open({name: "待审核江湖事", url: page.CONFIG.RELATION.WAIT});
...
//            if (page.assist.actionIsAdd()) url =  page.CONFIG.ADD_URL;
            ...
            $.ajax({
                ...
                success : function(ret) {
                    if(ret.code == 0){
                    	//console.log(page.CONFIG.RELATION.WAIT);
                    	//helper.win.open({name: "待审核江湖事", url: page.CONFIG.RELATION.WAIT});
                    	helper.win.changeQuoto({msg: "操作成功!", relation: page.CONFIG.RELATION.WAIT});
...
//            data.image = page.coverUpload.getFiles()[0];
...
//        data.related_obj_id = articleData.obj_id;
//        data.related_obj_type = articleData.obj_type;
//        data.related_title = articleData.article_title;
...
//                $("#txtSummary").val(data.summary);
                page.editor.setValue(data.NOTE);
                //上传（图片）组件初始化
