...
page.CONFIG = {
     //获取匿名主题的接口
    GET_INFO_API: helper.url.getUrlByMapping("admin/shanghui/find_information_detail.shtml"), //获取商会通知接口
...
//                    page.info = module.push.dataTransForm(res.data, res.pushdevice);
                ...
                toastr.error("您查看的商会动态不存在或发生错误!", "请稍候再重试或联系管理员！");
            },
            error: function () {
                toastr.error("您查看的商会动态不存在或发生错误!", "请稍候再重试或联系管理员！");
            }
    ...
    delete: function (status) {
        ...
        swal({
            title: "您确定要{0}该动态吗？".Format(label),
            type: "warning",
...
page.assist = {
    ...
    initDataBind: function () {
        ...
        console.info('发布时间',data.AUTHOR);
        //创建者
        var template = '&nbsp;&nbsp;<i class="glyphicon glyphicon-user"></i>&nbsp;<span>{0}</span>&nbsp;&nbsp;{1}&nbsp;{2}';
        var html = template.Format(data.AUTHOR, '发布于', helper.convert.formatDate(data.RELEASETIME.time));
