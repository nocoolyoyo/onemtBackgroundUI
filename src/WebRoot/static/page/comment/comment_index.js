...
require(['base', 'jquery', 'helper', 'sweetalert', 'layer', 'toastrPlus', 'module.push', 'datetimepicker', 'table', 'tableEditable', 'validator'], function (bs, $, helper, swal, layer, toastr) {
	...
	page.ajaxUrl = {
	    GET_LIST: helper.url.getUrlByMapping("admin/common/find_comment.shtml?status=1"),                   //查询所有评论接口
	    SELECT: helper.url.getUrlByMapping("admin/common/find_comment_detail.shtml"),                       //根据ID查询评论列表
	...
	page.eventHandler = {
	    ...
	    setEssence: function (id) {
	    	...
	    	swal({
	            ...
	            swal({title: "设为精彩评论中，请稍候", type: "info", showConfirmButton: false});
	            $.ajax({
	                ...
	                data: {
	                    ...
	                    extra: page.getCommentInfo(rowDate),
	       			    ...
	       			    user_id: rowDate.user_id,
	       			    title: rowDate.content,
	       			    summary: rowDate.content
	                },
	                dataType : 'json',
	                success : function(ret) {
	                    if(ret.code == 0){
	                    	swal({title:"设为精彩评论成功", text: "1s后自动消失...", type: "success", timer: 1000});
	                        page.$table.bootstrapTable('removeByUniqueId', id);
	                        page.$table.bootstrapTable('refresh');
	                    }else{
	                        swal("设为精彩评论失败", ret.errMsg, "error");
	                    }
	                },
	                error:function(ret) {
	                    swal("设为精彩评论失败", "error");
	    ...
	    cancelEssence: function (id) {
	    	...
	    	swal({
	            ...
	            swal({title: "取消精彩评论中，请稍候", type: "info", showConfirmButton: false});
	            $.ajax({
	                url: page.ajaxUrl.UPDATE,
	                type : 'POST',
	                data: {
	                ...
	                success : function(ret) {
	                    if(ret.code == 0){
	                    	swal({title:"取消精彩评论成功", text: "1s后自动消失...", type: "success", timer: 1000});
	                        page.$table.bootstrapTable('removeByUniqueId', id);
	                        page.$table.bootstrapTable('refresh');
	                    }else{
	                        swal("取消精彩评论失败", ret.errMsg, "error");
	                    }
	                },
	                error:function(ret) {
	                    swal("取消精彩评论失败", "error");
	...
	page.$table.bootstrapTable({
	    ...
	    columns: [{
	    	...
	    	width: '200px',
	    	formatter: function (value, row) {
	    		//return page.derive.setType(row.obj_type) + subStr(HTMLDecode(value), 35);
	    		var _title = value ? value : '';
	    		var _html = _title;
	    		//if (row.obj_type == 26) _html = row.user_name + '点赞了' + _title;
	    		if (row.obj_type == 38) _html = row.user_name + '关注了' + _title;
	    		//if (row.obj_type == 28) _html = row.user_name + '评论了' + _title;
	    		return '【'+ helper.obj.getObjLabel(row.obj_type) +'】' + _html;
	    	},
	        ...
	        formatter:function(value, row){
	        	var strHtml = '';
	        	row.is_good == 0 ? strHtml+=' <button type="button" class="btn btn-sm btn-info" onclick="page.eventHandler.setEssence(' + row.id + ')">设为精华</button>' : strHtml+=' <button type="button" class="btn btn-sm btn-info" onclick="page.eventHandler.cancelEssence(' + row.id + ')">取消精华</button>';
