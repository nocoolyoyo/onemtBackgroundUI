...
//页面所用到配置
page.CONFIG = {
    GET_LIST_API: helper.url.getUrlByMapping("admin/circle_share/find_share_list.shtml?state=1&status=1"),    //查询已发布帖子列表接口
    SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/circle_share/audit_share.shtml"),    				  //取消发布接口（帖子审核接口）
    DELETE_API: helper.url.getUrlByMapping("admin/circle_share/update_share.shtml"),                     	  //删除帖子接口
    REWRITE_API: helper.url.getUrlByMapping("admin/circle/update_circle_common.shtml"),                       //修改文章活动数据(点赞等数量)
    GOOD_API: helper.url.getUrlByMapping("admin/circle/update_circle_related.shtml"),                       //设为精华
    ...
    HANDLE_URL: helper.url.getUrlByMapping("admin/circlesharehandle.shtml?action=release"),                   //修改帖子url
    SHOW_URL: helper.url.getUrlByMapping("admin/circleshareinfo.shtml?action=release"),                       //查看url
    OBJ_TYPE: 19,            //帖子的类型id
...
//	                    $(elem).attr('disabled', false);
    ...
    //打开查看
    show: function (id) {
        helper.win.open({name: "查看帖子", url: page.CONFIG.SHOW_URL + "&id=" + id});
    },

    //打开编辑
    showEdit: function (id) {
        helper.win.open({name: "编辑帖子", url: page.CONFIG.HANDLE_URL + "&id=" + id});
    },
    ...
    showPush: function (id) {
        ...
        layer.open({
            type: 1,
            title: '<span class="label label-warning">【帖子】{0}</span>&nbsp;&nbsp;的再次推送'.Format(rowDate.title),
    ...
    doHandler: function (id, data, api, label) {
        var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
        swal({
            title: "您确定要{0}选中的帖子吗？".Format(label),
            text: rowDate.title,
            type: "warning",
