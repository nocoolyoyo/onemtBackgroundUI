...
//页面所用到配置
page.CONFIG = {
    GET_LIST_API: helper.url.getUrlByMapping("admin/topic/find_topic.shtml"),                     		//查询话题列表接口
    UPDATE: helper.url.getUrlByMapping("admin/circle/update_circle_related.shtml"),  					//设置文章状态接口(置顶，精华，删除) 
    SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/topic/audit_topic.shtml"),     					//提交审核接口
    DELETE_API: helper.url.getUrlByMapping("admin/topic/update_topic.shtml"),                        	//删除话题接口
    HANDLE_URL: helper.url.getUrlByMapping("admin/circleTopicHandle.shtml"),                   //修改圈子话题url
    SHOW_URL: helper.url.getUrlByMapping("admin/circletopicinfo.shtml"),                       //查看圈子话题url
    OBJ_TYPE: 5,             //话题的对象类型
    ...
    TYPE_LIST: {"wait": {type: "wait", param: "status=1&state=2", title: "待审核话题", action: "audit"},
        "draft": {type: "draft", param: "status=1&state=3,4", title: "话题草稿箱", action: "edit"},
        "trash": {type: "trash", param: "status=2", title: "话题垃圾箱", action: ""}
    }      //页面全部状态
};
...
//        helper.win.openAddByObj({obj_type: page.CONFIG.OBJ_TYPE});
    	helper.win.open({name: "新增话题", url: page.CONFIG.HANDLE_URL});
    },
...
//        method({obj_id: id, obj_type: page.CONFIG.OBJ_TYPE, article_title: rowDate.title});
        helper.win.open({name: "审核话题", url: page.CONFIG.SHOW_URL + "?action=" + page.typeInfo.action + "&id=" + id});
    },
...
//        helper.win.openEditByObj({obj_id: id, obj_type: page.CONFIG.OBJ_TYPE, article_title: rowDate.title, params: {action: action}});
        helper.win.open({name: "编辑话题", url: page.CONFIG.HANDLE_URL + "?action=" + page.typeInfo.action + "&id=" + id});
    },
