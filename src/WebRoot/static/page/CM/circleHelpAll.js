...
//页面所用到配置
page.CONFIG = {
    GET_LIST_API: helper.url.getUrlByMapping("admin/circle/find_question.shtml?status=1"),    	//查询已发布圈子帮助列表接口
    DELETE_API: helper.url.getUrlByMapping("admin/circle/update_circle_related.shtml"),                     //删除圈子帮助接口
    REWRITE_API: helper.url.getUrlByMapping("admin/circle/update_circle_common.shtml"),                     //修改文章圈子帮助数据(点赞等数量)
    UPDATE: helper.url.getUrlByMapping("admin/circle/update_circle_related.shtml"),      					//设文章状态接口(置顶，精华，删除)  
    PUSH_API: helper.url.getUrlByMapping("admin/common/commont_push.shtml"),                                //推送的接口
    COMMENT_URL: helper.url.getUrlByMapping("admin/commentindex.shtml"),                                    //评论管理
    SHOW_URL: helper.url.getUrlByMapping("admin/CMcircleHelpInfo.shtml"),                      //查看url
    HANDLE_URL:helper.url.getUrlByMapping("admin/CMcircleHelpHandle.shtml"),                      //编辑url
...
page.derive = {
    ...
    getParams: function (params) {
        ...
        params.create_start_time = helper.convert.formatTimestamp($("#releaseStartTime").val());
        params.create_end_time = helper.convert.formatTimestamp($("#releaseEndTime").val(), {day: 1});
...
page.eventHandler = {
    ...
    showComment: function (id) {
        ...
        var title = rowDate.content.substr(0, 5) + "..评论";
        helper.win.open({name: title, url: page.CONFIG.COMMENT_URL + "?obj_id=" + id + "&obj_type=" + page.CONFIG.OBJ_TYPE});
    },

    //打开查看
    show: function (id) {
        helper.win.open({name: "查看圈子帮助", url: page.CONFIG.SHOW_URL + "?action=release&id=" + id});
    },
    //设置/取消精华
    ...
    showEdit: function (id) {
        helper.win.open({name: "编辑圈子帮助", url: page.CONFIG.HANDLE_URL + "?action=release&id=" + id});
    },
    ...
    showPush: function (id) {
        ...
        var template = '<div class="wrapper wrapper-content animated full-height">'
            ...
            + '<div class="col-md-offset-2 col-md-3"><button class="btn btn-primary btn-block btn-lg" onclick="page.derive.doPush({id})"><i class="glyphicon glyphicon-ok"></i>&nbsp;&nbsp;确认推送</button></div>'
        ...
        layer.open({
            type: 1,
            title: '<span class="label label-warning">【圈子帮助】{0}</span>&nbsp;&nbsp;的再次推送'.Format(rowDate.title),
    ...
    doHandler: function (id, data, api, label) {
        var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
        swal({
            title: "您确定要{0}选中的圈子帮助吗？".Format(label),
            text: rowDate.title,
            type: "warning",
