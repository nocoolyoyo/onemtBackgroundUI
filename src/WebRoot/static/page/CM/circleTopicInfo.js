    ...
    initPageInfo: function () {
        $.ajax({
            ...
            success: function (res) {
                ...
                toastr.error("您查看的话题不存在或发生错误!", "请稍候再重试或联系管理员！");
            },
            error: function () {
                toastr.error("您查看的话题不存在或发生错误!", "请稍候再重试或联系管理员！");
            }
...
//            data.guest = page.info.data.guest;
    ...
    delete: function (status) {
        ...
        swal({
            title: "您确定要{0}该话题吗？".Format(label),
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#DD6B55",
...
//        }
        ...
        for(var i=0;i<circleList.length;i++){
                if(index === 1){
                    index++;
                    template +='<div class="label label-primary label-item">'+circleList[i].title+'</div>';
                }else if(index === 2){
                    index++;
                    template +='<div class="label label-warning label-item">'+circleList[i].title+'</div>';
                }else if(index === 3){
                    index++;
                    template +='<div class="label label-info label-item">'+circleList[i].title+'</div>';
                }else if(index === 4){
                    index++;
                    template +='<div class="label label-danger label-item">'+circleList[i].title+'</div>';
                }else if(index === 5){
                    index=1;
                    template +='<div class="label label-success label-item">'+circleList[i].title+'</div>';
                }
        }
        $("#lblExampleCircle").html(template);
        
        var guestList =data.guest;
        if(guestList.length>0){
            var str="";
            str +="<tr><td width='200' style='font-weight:bold;'>姓名</td><td width='250' style='font-weight:bold;'>用户身份</td></tr>"
            for(var i=0;i<guestList.length;i++){
            	str +="<tr><td>"+guestList[i].user_name+"</td><td>"+guestList[i].user_identity+"</td><td></tr>";
            }
...
//        console.log(data);
        //文章选择组件初始化
        page.linkSelector = new module.articleSelector({
      	  container: "#linkContainer",
      	  validatorContainer: "",
      	  readonly: true,
      	  isUnique: true,
      	  articleList: data.relatedUrl,
      	  callback: function (data) {
      	  }
      	});
        console.info(data.related)
        page.articleSelector = new module.articleSelector({
    	  container: "#articleContainer",
    	  validatorContainer: "",
    	  readonly: true,
    	  isUnique: true,
    	  articleList: data.related,
    	  callback: function (data) {
    	  }
    	});
