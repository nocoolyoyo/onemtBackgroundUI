...
//页面所用到配置
page.CONFIG = {
    GET_LIST_API: helper.url.getUrlByMapping("admin/activity/find_activity.shtml?state=1&status=1"),    	//查询已发布活动列表接口
    SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/activity/audit_activity.shtml"),    			//取消发布接口（活动审核接口）
    DELETE_API: helper.url.getUrlByMapping("admin/activity/update_activity.shtml"),                        //删除活动接口
    ...
    HANDLE_URL: helper.url.getUrlByMapping("admin/circleActivityHandle.shtml?action=release"),                  //修改活动url
    SHOW_URL: helper.url.getUrlByMapping("admin/circleActivityInfo.shtml?action=release"),                      //查看url
    Live_URL: helper.url.getUrlByMapping("admin/CMcircleActivityLive.shtml"),                      			//查看直播url
    OBJ_TYPE: 4,            //活动的类型id
...
page.derive = {
    ...
    getParams: function (params) {
        ...
        params.start_start_time = helper.convert.formatTimestamp($("#releaseStartTime").val());
        params.start_end_time = helper.convert.formatTimestamp($("#releaseEndTime").val(), {day: 1});
        ...
        params.circle_title = $("#circleKeyword").val();
    ...
    doPush: function (id) {
        ...
        if (bv.isValid()) {
            ...
            var data = {
                ...
                is_live:rowDate.is_live,
                is_start:rowDate.is_start,
                circle_nr_ids:rowDate.circle_nr_ids,
                circle_nr_names:rowDate.circle_title
            };
...
page.eventHandler = {
    ...
    //打开直播
    showLive: function (id) {
        var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
        var title = rowDate.title.substr(0, 5) + "..的直播";
        helper.win.open({name: title, url: page.CONFIG.Live_URL + "?page_id=" + id + "&page_title=" + rowDate.title});
    },

    //打开查看
    show: function (id) {
        helper.win.open({name: "查看活动", url: page.CONFIG.SHOW_URL + "&id=" + id});
    },
    //设置/取消精华
    setGood: function (id,index) {
                ...
                success : function(ret) {
                    if(ret.code == 0){
                    	...
                    	page.$table.bootstrapTable('updateRow', {index: index, row: row});
                      	$(this).text('取消精华').removeClass('btn-outline');
    		...
    		$.ajax({
                ...
                success : function(ret) {
                    if(ret.code == 0){
                    	row.is_good = 0;
                    	page.$table.bootstrapTable('updateRow', {index: index, row: row});
                    	$(this).text('设为精华').addClass('btn-outline');
    ...
    showEdit: function (id) {
        helper.win.open({name: "编辑活动", url: page.CONFIG.HANDLE_URL + "&id=" + id});
    },
    ...
    showPush: function (id) {
        ...
        layer.open({
            type: 1,
            title: '<span class="label label-warning">【活动】{0}</span>&nbsp;&nbsp;的再次推送'.Format(rowDate.title),
    ...
    doHandler: function (id, data, api, label) {
        var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
        swal({
            title: "您确定要{0}选中的活动吗？".Format(label),
            text: rowDate.title,
            type: "warning",
    ...
    page.$table.bootstrapTable({
        ...
        columns: [{
            ...
            formatter: function(value, row, index){
            	if(value=="1")
            		return '<span class="label label-info">直播中</span>';
            	else{
            		if(row.start_time>page.timestamp)
            			return '<span class="label label-warning">未开始</span>';
            		else if(row.end_time<page.timestamp){
                ...
                switch(value){
                    case 0: return '<span class="label label-warning">未开始</span>';break;
                    case 1: return '<span class="label label-primary">直播中</span>';break;
                    case 2: return '<span class="label label-danger">已结束</span>';break;
                    default: return "";
                }
            }
        },{
            ...
            formatter:function(value, row, index){
                ...
                strHtml += '&nbsp;<button type="button" class="btn btn-sm btn-warning" onclick="page.eventHandler.showLive(' + row.id + ')">直播</button>';
                strHtml += '&nbsp;<button type="button" class="btn btn-sm btn-warning" onclick="page.eventHandler.showEdit(' + row.id + ')">编辑</button>';
                strHtml += row.is_good===0?'&nbsp;<button type="button" class="btn btn-sm btn-outline btn-warning" onclick="page.eventHandler.setGood(' + row.id + ','+index+')">设为精华</button>':
   			       		   '&nbsp;<button type="button" class="btn btn-sm btn-warning" onclick="page.eventHandler.setGood(' + row.id + ','+index+')">取消精华</button>';
                strHtml += row.author_type===0?'&nbsp;<button type="button" disabled class="btn btn-sm btn-warning" onclick="page.eventHandler.backout(' + row.id + ')">取消发布</button>':
                		   '&nbsp;<button type="button" class="btn btn-sm btn-warning" onclick="page.eventHandler.backout(' + row.id + ')">取消发布</button>';
//                strHtml += '&nbsp;<button type="button" class="btn btn-sm btn-warning" onclick="page.eventHandler.backout(' + row.id + ')">取消发布</button>';
    ...
    $("#circleKeyword").keyup(function (event) {
        if(event.keyCode == 13)
            $("#btnSearch").click();
    });
