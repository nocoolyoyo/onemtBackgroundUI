...
page.CONFIG = {
    ...
    UPDATE_URL: helper.url.getUrlByMapping("admin/circlesharehandle.shtml"),                   				//在审核状态进入修改帖子url
...
page.eventHandler = {
    ...
    initPageInfo: function () {
        $.ajax({
            ...
            success: function (res) {
                ...
                toastr.error("您查看的帖子不存在或发生错误!", "请稍候再重试或联系管理员！");
            },
            error: function () {
                toastr.error("您查看的帖子不存在或发生错误!", "请稍候再重试或联系管理员！");
            }
    ...
    delete: function (status) {
        ...
        swal({
            title: "您确定要{0}该帖子吗？".Format(label),
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#DD6B55",
...
page.assist = {
    ...
    initDataBind: function () {
        ...
        var html = template.Format(data.user_name, '创建于', helper.convert.formatDate(data.create_time));
        $("#divCreatePanel").html(html);
        //编辑者
        if(data.audit_user_name){
            html = template.Format(data.audit_user_name, '修改于', helper.convert.formatDate(data.update_time));
    ...
    showCircle: function () {
    	$("#lblCircle").html(page.info.data.circleList[0].title);
    },
