...
//页面所用到配置
page.CONFIG = {
    GET_LIST_API: helper.url.getUrlByMapping("admin/circle/find_question.shtml"),                     		//查询圈子帮助列表接口
    DELETE_API: helper.url.getUrlByMapping("admin/circle/update_circle_related.shtml"),                     //恢复圈子帮助接口
    SHOW_URL: helper.url.getUrlByMapping("admin/CMcircleHelpInfo.shtml"),                                     //查看/审核url
    ...
    TYPE_LIST: {"wait": {type: "wait", param: "status=1&state=2", title: "待审核圈子帮助", action: "audit"},
        "draft": {type: "draft", param: "status=1&state=3", title: "圈子帮助草稿箱", action: ""},
        "trash": {type: "trash", param: "status=2", title: "圈子帮助垃圾箱", action: ""}
    }      //页面全部状态
};

page.type = helper.url.queryString("type") || page.CONFIG.TYPE_LIST.trash.type;
...
page.eventHandler = {
    ...
    showAudit: function (id) {
        helper.win.open({name: "查看圈子帮助", url: page.CONFIG.SHOW_URL +"?action=trash&id=" + id});
    },
