...
//页面所用到配置
page.CONFIG = {
    GET_LIST_API: helper.url.getUrlByMapping("admin/topic/find_topic.shtml?state=1&status=1"),    			//查询已发布话题列表接口
    SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/topic/audit_topic.shtml"),    						//取消发布接口（话题审核接口）
    DELETE_API: helper.url.getUrlByMapping("admin/topic/update_topic.shtml"),                        		//删除话题接口
    ...
    HANDLE_URL: helper.url.getUrlByMapping("admin/circleTopicHandle.shtml?action=release"),                   //修改圈子话题url
    SHOW_URL: helper.url.getUrlByMapping("admin/circletopicinfo.shtml?action=release"),                       //查看圈子话题url
    OPINION_URL:"CMcircleTopicOpinionsAll.shtml", 						//观点管理url
    OBJ_TYPE: 5,            //话题的类型id
...
page.derive = {
    ...
    getParams: function (params) {
        ...
        params.audit_start_time = helper.convert.formatTimestamp($("#releaseStartTime").val());
        params.audit_end_time = helper.convert.formatTimestamp($("#releaseEndTime").val(), {day: 1});
...
page.eventHandler = {
    ...
    showComment: function (id) {
        ...
        helper.win.openCommentByObj({obj_id: id, obj_type: page.CONFIG.OBJ_TYPE, article_title: rowDate.title})
    },
    //话题管理
    showOpinion: function (id) {
        var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
        var title = rowDate.title.substr(0, 5) + "..的大咖观点";
    	helper.win.open({name: title, url: page.CONFIG.OPINION_URL + "?page_id=" + id});
    },
    //打开查看
    show: function (id) {
    	helper.win.open({name: "查看话题", url: page.CONFIG.SHOW_URL + "&id=" + id});
//        var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
//        helper.win.openAuditByObj({obj_id: id, obj_type: page.CONFIG.OBJ_TYPE, article_title: rowDate.title});
    },

    //打开编辑
    showEdit: function (id) {
    	helper.win.open({name: "编辑话题", url: page.CONFIG.HANDLE_URL + "&id=" + id});
//        var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
//        helper.win.openEditByObj({obj_id: id, obj_type: page.CONFIG.OBJ_TYPE, article_title: rowDate.title, params: {action: "release"}});
    },
    ...
    showPush: function (id) {
        ...
        layer.open({
            type: 1,
            title: '<span class="label label-warning">【话题】{0}</span>&nbsp;&nbsp;的再次推送'.Format(rowDate.title),
    ...
    doHandler: function (id, data, api, label) {
        var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
        swal({
            title: "您确定要{0}选中的话题吗？".Format(label),
            text: rowDate.title,
            type: "warning",
    ...
    page.$table.bootstrapTable({
        ...
        columns: [{
            ...
            formatter:function(value, row){
    			...
    			strHtml += '&nbsp;<button type="button" class="btn btn-sm btn-warning" onclick="page.eventHandler.showOpinion(' + row.id + ')">观点管理</button>';
