...
//页面所用到配置
page.CONFIG = {
    GET_LIST_API: helper.url.getUrlByMapping("admin/circle_share/find_share_list.shtml"),                     //查询帖子列表接口
    SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/circle_share/audit_share.shtml"),    			  //提交审核接口
    DELETE_API: helper.url.getUrlByMapping("admin/circle_share/update_share.shtml"),                       //删除帖子接口
    HANDLE_URL: helper.url.getUrlByMapping("admin/circlesharehandle.shtml"),                                   //新增/审核/修改帖子url
    SHOW_URL: helper.url.getUrlByMapping("admin/circleshareinfo.shtml"),                                       //查看/审核url
    ...
    TYPE_LIST: {"wait": {type: "wait", param: "status=1&state=2", title: "待审核帖子", action: "audit"},
        "draft": {type: "draft", param: "status=1&state=3,4", title: "帖子草稿箱", action: ""},
        "trash": {type: "trash", param: "status=2", title: "帖子垃圾箱", action: ""}
    }      //页面全部状态
};
...
page.derive = {
    ...
    getParams: function (params) {
        ...
        params.delete_start_time = helper.convert.formatTimestamp($("#delStartTime").val());
        params.delete_end_time = helper.convert.formatTimestamp($("#delEndTime").val(), {day: 1});
...
page.eventHandler = {
    //打开新增帖子
    showAdd: function () {
        helper.win.open({name: "新增帖子", url: page.CONFIG.HANDLE_URL});
    },

    //打开审核
    showAudit: function (id) {
        helper.win.open({name: "审核帖子", url: page.CONFIG.SHOW_URL + "?action=" + page.typeInfo.action + "&id=" + id});
    },

    //打开编辑
    showEdit: function (id) {
        var action = page.typeInfo.action ? page.typeInfo.action : "edit";
        helper.win.open({name: "编辑帖子", url: page.CONFIG.HANDLE_URL + "?action=" + action + "&id=" + id});
    },
