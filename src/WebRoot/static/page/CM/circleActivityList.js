...
//页面所用到配置
page.CONFIG = {
    GET_LIST_API: helper.url.getUrlByMapping("admin/activity/find_activity.shtml"),                     //查询活动列表接口
    SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/activity/audit_activity.shtml"),    		//提交审核接口
    DELETE_API: helper.url.getUrlByMapping("admin/activity/update_activity.shtml"),                        //删除活动接口
    HANDLE_URL: helper.url.getUrlByMapping("admin/circleActivityHandle.shtml"),                                 //新增/审核/修改活动url
    SHOW_URL: helper.url.getUrlByMapping("admin/circleActivityInfo.shtml"),                                     //查看/审核url
    ...
    TYPE_LIST: {"wait": {type: "wait", param: "status=1&state=2", title: "待审核活动", action: "audit"},
        "draft": {type: "draft", param: "status=1&state=3,4", title: "活动草稿箱", action: ""},
        "trash": {type: "trash", param: "status=2", title: "活动垃圾箱", action: ""}
    }      //页面全部状态
};
...
page.derive = {
    ...
    getParams: function (params) {
        ...
        params.startupdatetime = helper.convert.formatTimestamp($("#delStartTime").val());
        params.endupdatetime = helper.convert.formatTimestamp($("#delEndTime").val(), {day: 1});
...
page.eventHandler = {
    //打开新增活动
    showAdd: function () {
        helper.win.open({name: "新增活动", url: page.CONFIG.HANDLE_URL});
    },

    //打开审核
    showAudit: function (id) {
        helper.win.open({name: "审核活动", url: page.CONFIG.SHOW_URL + "?action=" + page.typeInfo.action + "&id=" + id});
    },

    //打开编辑
    showEdit: function (id) {
        var action = page.typeInfo.action ? page.typeInfo.action : "edit";
        helper.win.open({name: "编辑活动", url: page.CONFIG.HANDLE_URL + "?action=" + action + "&id=" + id});
    },
