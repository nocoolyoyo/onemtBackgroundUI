...
require(['base', 'jquery', 'helper', 'sweetalert', 'layer', 'toastrPlus', 'module.push', 'datetimepicker', 'table', 'tableEditable', 'validator'], function (bs, $, helper, swal, layer, toastr) {
	...
	page.ajaxUrl = {
	    GET_LIST: helper.url.getUrlByMapping("admin/topic/find_topic_guest_opinion.shtml?status=2"),        //获取全部话题观点接口
	    ...
	    ADD_OPINION: helper.url.getUrlByMapping("admin/CMcircleTopicOpinionsAdd.shtml")					//代发大咖观点
	}
	...
	page.$table.bootstrapTable({
	    ...
	    columns: [{
	        ...
	        width: "100px",
	        formatter:function(value, row){
	            var strHtml=' <button type="button" class="btn btn-sm btn-danger" onclick="page.eventHandler.delete(' + row.id + ')">恢复</button>';
	            return strHtml;
	        }
	    }]
	});
