var page = {};
require(['base', 'jquery', 'helper', 'sweetalert', 'toastr', 'layer', 'datetimepicker', 'table'], function (bs, $, helper, swal, toastr, layer) {
//表格初始化
var $table = $('#tableList');
//页面事件
page.eventHandler = {
	showAdd: function () {
		helper.win.open({name: "新增匿名主题",url: "anonymousadd.shtml"});
	},
	showUpdata: function (id) {
		helper.win.open({name: "编辑匿名主题",url: "anonymousedit.shtml?id="+id});
	},
	showDict: function (id) {
		var rowDate = $('#tableList').bootstrapTable('getRowByUniqueId', id);
		var title = rowDate.name.substr(0, 5) + "..的词典";
		helper.win.open({name: title,url: "anonymousdictlist.shtml?type="+rowDate.code});
	},
	delete: function (id) {
	    swal({
	        title: "您确定要删除选中的信息吗？",
    		...
    		$.ajax({
	    		url: 'anonymous/delete_anonymous.shtml',
	    		data: {
	    			id: id
	    		},
	    		type: 'post',
...
$table.bootstrapTable({
    //请求相关
    url: helper.url.getUrlByMapping('admin/anonymous/find_anonymouslists.shtml'),  //AJAX读取列表数据的URL
    method: "get",                  //请求方式
    ...
    columns: [{
        ...
        formatter: function(value){
        	var strHtml='';
    		strHtml+=' <button type="button" class="btn btn-sm btn-info"  onclick="page.eventHandler.showDict('+value+')">查看词典</button>';
    		strHtml+=' <a class="btn btn-sm btn-warning" onclick="page.eventHandler.showUpdata('+value+')">修改</a>';
