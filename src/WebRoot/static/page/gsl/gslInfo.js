...
require(['base', 'jquery', 'helper', 'toastr', 'sweetalert', 'module.push', 'fancybox'], function (bs, $, helper, toastr, swal) {
    ...
    //页面所用到配置
page.CONFIG = {
    GET_INFO_API: helper.url.getUrlByMapping("admin/gsl/find_gsl_detail.shtml"),                //获取工商联要闻详情接口
    AUDIT_API: helper.url.getUrlByMapping("admin/gsl/audit_gsl.shtml"),           				//工商联要闻审核接口
    DELETE_API: helper.url.getUrlByMapping("admin/gsl/update_gsl.shtml"),                        //删除工商联要闻接口
    UPDATE_URL: helper.url.getUrlByMapping("admin/gsladd.shtml"),                   			//在审核状态进入修改工商联要闻url
...
page.eventHandler = {
    ...
    initPageInfo: function () {
        $.ajax({
            ...
            success: function (res) {
                ...
                toastr.error("您查看的工商联要闻不存在或发生错误!", "请稍候再重试或联系管理员！");
            },
            error: function () {
                toastr.error("您查看的工商联要闻不存在或发生错误!", "请稍候再重试或联系管理员！");
            }
    ...
    delete: function (status) {
        ...
        swal({
            title: "您确定要{0}该工商联要闻吗？".Format(label),
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#DD6B55",
