...
require(['base', 'jquery', 'helper', 'sweetalert', 'layer', 'toastrPlus', 'module.push', 'datetimepicker', 'table', 'tableEditable', 'validator'], function (bs, $, helper, swal, layer, toastr) {
    ...
    //页面所用到配置
page.CONFIG = {
    GET_LIST_API: helper.url.getUrlByMapping("admin/gsl/find_gsl.shtml?status=1&state=1"),    //查询已发布工商联要闻列表接口
    SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/gsl/audit_gsl.shtml"),    			  //取消发布接口（工商联要闻审核接口）
    ...
    HANDLE_URL: helper.url.getUrlByMapping("admin/gsladd.shtml?action=release"),              //修改工商联要闻url
    SHOW_URL: helper.url.getUrlByMapping("admin/gslinfo.shtml?action=release"),               //查看url
    OBJ_TYPE: 9,            //工商联要闻的类型id
...
page.eventHandler = {
    ...
    //打开查看
    show: function (id) {
        helper.win.open({name: "查看工商联要闻", url: page.CONFIG.SHOW_URL + "&id=" + id});
    },

    //打开编辑
    showEdit: function (id) {
        helper.win.open({name: "编辑工商联要闻", url: page.CONFIG.HANDLE_URL + "&id=" + id});
    },
    ...
    showPush: function (id) {
        ...
        layer.open({
            type: 1,
            title: '<span class="label label-warning">【工商联要闻】{0}</span>&nbsp;&nbsp;的再次推送'.Format(rowDate.title),
    ...
    doHandler: function (id, data, api, label) {
        var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
        swal({
            title: "您确定要{0}选中的工商联要闻吗？".Format(label),
            text: rowDate.title,
            type: "warning",
