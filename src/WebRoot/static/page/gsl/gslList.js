...
//页面所用到配置
page.CONFIG = {
    GET_LIST_API: helper.url.getUrlByMapping("admin/gsl/find_gsl.shtml"),                     //查询工商联要闻列表接口
    SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/gsl/audit_gsl.shtml"),                //提交审核接口
    DELETE_API: helper.url.getUrlByMapping("admin/gsl/update_gsl.shtml"),                        //删除工商联要闻接口
    HANDLE_URL: helper.url.getUrlByMapping("admin/gsladd.shtml"),                                 //新增/审核/修改工商联要闻url
    SHOW_URL: helper.url.getUrlByMapping("admin/gslinfo.shtml"),                                     //查看/审核url
    ...
    TYPE_LIST: {"wait": {type: "wait", param: "status=1&state=2", title: "待审核工商联要闻", action: "audit"},
        "draft": {type: "draft", param: "status=1&state=3,4", title: "工商联要闻草稿箱", action: ""},
        "trash": {type: "trash", param: "status=2", title: "工商联要闻垃圾箱", action: ""}
    }      //页面全部状态
};
...
page.derive = {
    ...
    getParams: function (params) {
        ...
        params.audit_start_time = helper.convert.formatTimestamp($("#delStartTime").val());
        params.audit_end_time = helper.convert.formatTimestamp($("#delEndTime").val(), {day: 1});
...
page.eventHandler = {
    //打开新增工商联要闻
    showAdd: function () {
        helper.win.open({name: "新增工商联要闻", url: page.CONFIG.HANDLE_URL});
    },

    //打开审核
    showAudit: function (id) {
        helper.win.open({name: "审核工商联要闻", url: page.CONFIG.SHOW_URL + "?action=" + page.typeInfo.action + "&id=" + id});
    },

    //打开编辑
    showEdit: function (id) {
        var action = page.typeInfo.action ? page.typeInfo.action : "edit";
        helper.win.open({name: "编辑工商联要闻", url: page.CONFIG.HANDLE_URL + "?action=" + action + "&id=" + id});
    },
