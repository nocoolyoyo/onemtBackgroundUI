...
require(['base', 'jquery', 'helper', 'toastr', 'sweetalert', 'module.push', 'module.articleSelector', 'fancybox'], function (bs, $, helper, toastr, swal) {
    var module = {
  ...
  //页面所用到配置
    page.CONFIG = {
        GET_INFO_API: helper.url.getUrlByMapping("admin/article/get_article_id.shtml"),                //获取江湖事接口
        AUDIT_API: helper.url.getUrlByMapping("admin/article/audit_article.shtml"),                    //江湖事审核接口
        DELETE_API: helper.url.getUrlByMapping("admin/article/delete_article_id.shtml"),               //删除江湖事接口
        UPDATE_URL: helper.url.getUrlByMapping("admin/jhsHandle.shtml"),                               //在审核状态进入修改江湖事url
    ...
    page.eventHandler = {
        ...
        initPageInfo: function () {
            $.ajax({
                ...
                success: function (res) {
                    ...
                    toastr.error("您查看的江湖事不存在或发生错误!", "请稍候再重试或联系管理员！");
                },
                error: function () {
                    toastr.error("您查看的江湖事不存在或发生错误!", "请稍候再重试或联系管理员！");
                }
        ...
        delete: function (status) {
            ...
            swal({
                title: "您确定要{0}该江湖事吗？".Format(label),
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#DD6B55",
    ...
    page.assist = {
        ...
        initDataBind: function () {
            ...
            //垃圾箱状态
            var $status = $("#lblStatus");console.log(data.status);
            ...
            console.log(data);
            //文章内容
            var images = data.image.trim() ? data.image.split(",") : [];
            if(data.is_short == 0){
                //长文
                $("#typLength").show();
                $("#typShort").hide();
                ...
                if(images.length > 0){
                	$('#divLengthImagesCon').show();
                	template = '<a class="fancybox imgbox" rel="group" href="{0}" title=""><img alt="" style="width: 100%;height: 100%" src="{0}" /></a>';
                    html = '';
                    html += template.Format(images[0]);
                    $("#divLengthImages").html(html).find(".fancybox").fancybox();
                }
                ...
                if(images.length > 0){
                    template = '<a class="fancybox imgbox mg-10" rel="group" href="{0}" title=""><img alt="" style="height: 100%; width: 100%" src="{0}" /></a>';
                    html = '';
            ...
            page.articleSelector = new module.articleSelector({
        	  ...
        	  readonly: true,
        	  isUnique: true,
        	  articleList: page.assist.setArticle(data.relatelist),
        ...
        //变更页面按钮的状态（根据页面状态）
        setButtonShow: function () {console.log(page.action);
