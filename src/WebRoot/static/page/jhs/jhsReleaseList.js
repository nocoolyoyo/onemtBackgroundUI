...
require(['base', 'jquery', 'helper', 'sweetalert', 'layer', 'toastrPlus', 'module.push', 'datetimepicker', 'table', 'tableEditable', 'validator'], function (bs, $, helper, swal, layer, toastr) {
  ...
  //页面所用到配置
    page.CONFIG = {
        GET_LIST_API: helper.url.getUrlByMapping("admin/article/find_article.shtml?status=1&state=1"),          //查询已发布江湖事列表接口
        SUBMIT_AUDIT_API: helper.url.getUrlByMapping("admin/article/audit_article.shtml"),                      //取消发布接口（江湖事审核接口）
        ...
        HANDLE_URL: helper.url.getUrlByMapping("admin/jhsHandle.shtml?action=release"),                         //修改江湖事url
        SHOW_URL: helper.url.getUrlByMapping("admin/jhsInfo.shtml?action=release"),                             //查看江湖事url
        OBJ_TYPE: 7,            //江湖事的类型id
    ...
    page.eventHandler = {
        ...
        showPush: function (id) {
            ...
            layer.open({
                type: 1,
                title: '<span class="label label-warning">【江湖事】{0}</span>&nbsp;&nbsp;的再次推送'.Format(rowDate.title),
        ...
        doHandler: function (id, data, api, label) {
            var rowDate = page.$table.bootstrapTable('getRowByUniqueId', id);
            swal({
                title: "您确定要{0}选中的江湖事吗？".Format(label),
                text: rowDate.title,
                type: "warning",
        ...
        page.$table.bootstrapTable({
            ...
            columns: [{
                ...
                formatter: function(value, row){
                	switch(row.is_short){
                	    case 0: return '<a href="javascript:;" onclick="page.eventHandler.show('+row.id+')">'+ row.title.substr(0,35) +'</a>';
                	    case 1: return '<a href="javascript:;" onclick="page.eventHandler.show('+row.id+')">'+ row.content.substr(0,35) +'</a>';
                        default: return '-';
                    }
                }
            },{
